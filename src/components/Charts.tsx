...
export const Charts: React.FC = () => {
  ...
  // Data for Pie Chart (Expenses by Category) - Filtered by Month
  ...
  // Data for Bar Chart (Income vs Expense per month) - Keep History but maybe highlight?
  // Keeping as is for context (6 months history is always good).
  ...
  const forecastData = React.useMemo(() => {
    const today = new Date(); // Start from "today/next month" or just future
    // Actually user wants "Forecast of expenses". 
    // Let's list all FUTURE installments and ACTIVE fixed expenses for next 12 months.
    ...
    const items: Array<{
        ...
        installmentInfo?: string; // e.g. "2/10"
    }> = [];
    ...
    // 2. Installments (Actually look for transactions with future dates)
    transactions.filter(t => {
        ...
        items.push({
            date: tDate,
            description: t.description.replace(/\(\d+\/\d+\)/, '').trim(), // Remove old auto-label if exists to avoid double (2/10) (2/10)
            amount: t.amount,
    ...
    <div className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 h-96 transition-colors">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Gastos por Categoria (Mês Selecionado)</h3>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 h-96 transition-colors">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Evolução Mensal (Histórico)</h3>
                <ResponsiveContainer width="100%" height="85%">
                <BarChart
                    data={monthlyData}
